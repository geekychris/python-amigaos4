...
#define SEP_OR_END(x) (IS_SEP(x) || IS_END(x))
    ...
    // Skip past drive/volume segment and update minP2
    else if (p1[0] && wcschr(p1, L':') != NULL) {
        wchar_t *colon = wcschr(p1, L':');
        wchar_t *slash = wcschr(p1, L'/');
        wchar_t *bslash = wcschr(p1, L'\\');
        if (colon && (!slash || colon < slash) && (!bslash || colon < bslash)) {
            while (p1 <= colon) {
                *p2++ = *p1++;
            }
            minP2 = p2;
