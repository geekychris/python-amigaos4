...
for v in "${variants[@]}"; do
    ...
    # clib4 REQUIRES PYTHONUTF8=1 because its setlocale returns
    # "C-ISO-8859-1" which throws off CPython's pyconfig_calculate
    # wchar buffer sizing. UTF-8 mode bypasses that code path.
    # Newlib returns plain "C" and doesn't need the workaround.
    extra_env=""
    if [ "$v" = "clib4" ]; then
        extra_env="setenv PYTHONUTF8 1"$'\n'
    fi
    cat > "$TMPW" <<EOF
; python-os4 launcher — $v variant (auto-generated by install-on-guest.sh)
setenv PYTHONHOME DH1:
setenv PYTHONPATH "DH1:lib"
${extra_env}$GUEST_DIR/python-os4 \$@
EOF
    _push "$TMPW" "DH1:py$v"
