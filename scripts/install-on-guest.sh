...
for v in "${variants[@]}"; do
    echo "─── $v ─────────────────────────────────────────────────────"
    # Binaries are fully statically linked (readelf -d shows "no dynamic
    # section"). No .so files need to be co-located — clib4's libc.so /
    # libpthread.so etc are only needed if you dlopen a Python C
    # extension .so at runtime, and our build has ac_cv_func_dlopen=no.
    # Only clib4.library in LIBS: is required at boot for clib4 binaries.
    case "$v" in
        newlib)
            BDIR="$REPO/build-ppc-amigaos-750"
            EXE="$BDIR/python-stripped.exe"
            [ -f "$EXE" ] || EXE="$BDIR/python.exe"
            _push "$EXE" "DH1:python-os4-newlib"
            ;;
        clib4)
            ...
            EXE="$BDIR/python-stripped.exe"
            [ -f "$EXE" ] || EXE="$BDIR/python.exe"
            _push "$EXE" "DH1:python-os4-clib4"
...
for v in "${variants[@]}"; do
    case "$v" in
        newlib)
            _push "$REPO/scripts/amiga-scripts/pynewlib" "DH1:pynewlib"
            _push "$REPO/scripts/amiga-scripts/setup-python-os4-newlib" \
                  "DH1:setup-python-os4-newlib"
            _dos "protect DH1:pynewlib rwed"
            _dos "protect DH1:setup-python-os4-newlib rwed"
            ;;
        clib4)
            _push "$REPO/scripts/amiga-scripts/pyclib4" "DH1:pyclib4"
            _push "$REPO/scripts/amiga-scripts/setup-python-os4-clib4" \
                  "DH1:setup-python-os4-clib4"
            _dos "protect DH1:pyclib4 rwed"
            _dos "protect DH1:setup-python-os4-clib4 rwed"
            ;;
    esac
done
